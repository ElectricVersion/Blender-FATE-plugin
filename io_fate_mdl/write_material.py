...
def write_material_section(wtr):
    for i in range(len(wtr.mdlData.textures)):
        #texture names
        wtr.write_str("TEST") #FIX THIS LATER ONCE I CAN ACCESS THE DOCS
    ...
    for i in range(len(wtr.mdlData.materials)):
        ...
        wtr.write_num(255, UINT16) #diffuse R
        wtr.write_num(255, UINT16) #diffuse G
        wtr.write_num(255, UINT16) #diffuse B
                
        wtr.write_num(0, UINT16) #ambient R
        wtr.write_num(0, UINT16) #ambient G
        wtr.write_num(0, UINT16) #ambient B        
